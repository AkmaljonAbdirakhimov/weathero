import 'package:weathero/core/extensions/map_weather_code_extensions.dart';
import 'package:weathero/core/helper/date_helper.dart';
import 'package:weathero/core/helper/location_helper.dart';
...
class HourlyForecastCubit extends Cubit<DetailedForecastState> {
  ...
  fetchWeatherData() async {
    WeatherNewsModel article = WeatherNewsModel();
    var weatherNews = await _repository.fetchWeatherNews();
    weatherNews.fold(
        ...
        emit(
          DetailsForecastSuccess(
            ...
            dailyForecast: Daily(
              ...
              date:
                  DateHelper.formatDate(response.daily.time[0], 'EEEE, d MMMM'),
              sunrise:
              ...
              sunset: DateHelper.formatDate(response.daily.sunset[0], 'h:mm a'),
              apparentTemperature: (response.daily.apparentTemperatureMax[0] +
                      response.daily.apparentTemperatureMin[0]) ~/
                  2,
              humidity: response.hourly.humidity[0],
              windSpeed: response.daily.windSpeed[0],
            ),
